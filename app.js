...
async function editNpcProfile(name) {
  ...
  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';
  overlay.innerHTML = `<div class="modal-box" style="max-width:500px;max-height:85vh;overflow-y:auto"><div style="font-size:15px;font-weight:500;margin-bottom:12px">${name?'编辑':'新建'}角色档案</div><label class="api-field">名称<input id="np-name" type="text" value="${escapeHtml(profile.name)}" style="width:100%"></label><label class="api-field">外观<textarea id="np-appearance" class="world-setting" rows="2" style="width:100%">${escapeHtml(profile.appearance||'')}</textarea></label><label class="api-field">性格<textarea id="np-personality" class="world-setting" rows="2" style="width:100%">${escapeHtml(profile.personality||'')}</textarea></label><label class="api-field">关系（JSON）<textarea id="np-rel" class="world-setting" rows="2" style="width:100%">${escapeHtml(JSON.stringify(profile.relationships||{},null,2))}</textarea></label><label class="api-field">备注<textarea id="np-notes" class="world-setting" rows="2" style="width:100%">${escapeHtml(profile.notes||'')}</textarea></label><div style="margin-top:16px;display:flex;justify-content:flex-end;gap:8px"><button id="np-cancel" class="btn-sm" style="padding:6px 16px">取消</button><button id="np-save" class="btn-sm" style="padding:6px 16px;background:var(--accent);color:#fff;border:none;border-radius:6px">保存</button></div></div>`;
  document.body.appendChild(overlay);
